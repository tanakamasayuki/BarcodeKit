...
| オブジェクトのサイズ | 実測 64 バイト（Codabar だけ設定が多く 72 バイト）。うち 49 バイトが `text()` 用バッファで、`BARCODEKIT_TEXT_MAX=16` にすると 32 バイトになる。いずれも 64bit ホストでの値で、AVR ではポインタが 2 バイトのぶん小さい |
...
- `setCheckDigit(true)` は桁数の偶奇を変えるため、`setPadOdd(true)` との併用を前提に「チェックディジット付加 → 奇数なら先頭に 0」の順で確定させる（先頭 0 は mod 10 の重みに影響しないので順序による差は出ない）。
- 入力は 32 桁まで（`kMaxDigits`）。`encode()` が桁をいったんスタックに置くため、上限を決めておかないとスタック使用量が入力長に比例してしまう。
...
- チェックディジットは規格上任意。既定なし、`setCheckDigit(true)` で mod 16 を付加する。ただし **Codabar のチェックディジットは規格の一部ではなく運用ごとに規約が異なり、デコーダも検証しない**（テストで正しさを裏付けられない唯一の箇所）。採用した規約と、その旨を [FORMATS.ja.md](FORMATS.ja.md) に明記している。
- narrow:wide 比は `setRatio()`、既定 **3**。
