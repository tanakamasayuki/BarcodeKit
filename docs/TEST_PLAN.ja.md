...
| `determinism/` | あり | 同じ入力を 2 つのオブジェクト・2 つのバッファで符号化して全モジュールが一致すること、10 回繰り返しても変わらないこと、オブジェクトを再利用しても前回の入力が残らないこと、失敗後に状態が消えること。QR は自動マスク選択が毎回同じであることも見る |
| `fuzz/` | あり | ランダム入力で範囲外書き込み・不正な結果・停止が起きないこと（§3.1） |
| `noalloc/` | あり | `malloc` / `calloc` / `realloc` / `free` と `new` / `delete` を計数版に差し替え、`encode()` 中と結果の読み出し中に 1 回も呼ばれないことを確認 |

これらは [REQUIREMENTS.ja.md](REQUIREMENTS.ja.md) §8 の「再現性」「安定性」「メモリ効率」を裏付ける。

`noalloc/` は**フック自体が効いているかを最初に確認する**。差し替えが効いていなければ計数は常に 0 のままで、以降のすべてが誤った理由で成功してしまうため。意図的に 1 回確保して計数が動くことを確かめてから本題に入る。

### 3.1 `fuzz/` だけ作りが違う

`fuzz/` は Arduino ツールチェーンを通さず、**g++ で直接ビルドして実行する**。価値の大半が ASan / UBSan にあるが、`sketch.yaml` のプロファイルにはコンパイルフラグを渡す場所が無いため。ライブラリは Arduino のヘッダを必要としないので、ホストビルドだけで足りる。

- 乱数は固定シードの xorshift。失敗をシードから再現できないファジングは価値が落ちるため
- 形式を先に選び、**その形式が受け付けそうな入力を作ってから 1/4 の確率で壊す**。完全な乱数バイトはほぼ検証を通らず、符号化そのものが動かない（実際、最初の実装は 20,000 回中 73 回しか符号化に到達していなかった。現在は約半数）
- バッファはガードバイトで挟み、**渡した領域の外に一切書かないこと**を毎回確認する
- 成功したシンボルは全モジュールを読み、範囲外座標が `false` を返すこと、`text()` が上限内であること、描画ヘルパーが例外なく完走することまで見る
